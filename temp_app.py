...
# Directories
save_directory = "images\\image2"  # Directory for saving images
temp_image_dir = "temp_img\\temp_img2"  # Directory for temporary image storage
...
# Model paths
torpedo_classification_model_path = "E:\\flir\\Torpedo_Detection\\models\\cam2_classify.pt"
cam1_detection_model_path = "E:\\flir\\Torpedo_Detection\\models\\cam2_detect.pt"

# Load models
torpedo_classification_model = YOLO(torpedo_classification_model_path)
cam1_detection_model = YOLO(cam1_detection_model_path)

# Tracking for single torpedo detection
class TorpedoDetectionTracker:
    def __init__(self, detection_interval=300):  # 5 minutes default
    ...
    def can_record_detection(self):
        """
        Check if a new detection can be recorded based on time interval.

        Returns:
            bool: True if detection can be recorded, False otherwise
        """
        current_time = datetime.datetime.now()

        # If no previous detection, allow recording
    ...
    def record_detection(self, detection_data):
        """
        Record the first detection and update last detection time.

        Args:
            detection_data (dict): Detection information

        Returns:
            bool: True if detection is recorded, False otherwise
        """
...
def insert_detection_data(torpedo_tracker, torpedo_id, c_x, c_y, w, h, cam_id, filename):
    """
    Send detection data to the API endpoint with single detection tracking.

    Args:
        torpedo_tracker (TorpedoDetectionTracker): Tracker to manage single detection
    ...
    # Check if this detection can be recorded
...
def main():
    ...
    torpedo_id = 100
    torpedo_tracker = TorpedoDetectionTracker(detection_interval=300)  # 5 minutes
    ...
    def cam1_detection_thread():
        ...
        # Use torpedo tracker to manage single detection
